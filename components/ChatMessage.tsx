      ...
      <div className={`max-w-[95%] sm:max-w-[85%] flex flex-col ${isUser ? 'items-end' : 'items-start'}`}>
        ...
        {!isUser && message.reasoning && (
          <div className="mb-3 w-full animate-fade-in">
            ...
            >
              <div className={`text-[10px] font-black tracking-widest uppercase ${showReasoning ? 'text-cyan-400' : 'text-slate-500'}`}>
            ...
            {showReasoning && (
              <div className="mt-2 p-4 bg-[#161b22] border-l-2 border-cyan-600 text-slate-400 text-[13px] italic font-light leading-relaxed rounded-r-xl shadow-inner overflow-hidden">
                <div className="flex items-center gap-2 mb-2 opacity-50">
                  <div className="w-1.5 h-1.5 bg-cyan-500 rounded-full animate-pulse"></div>
                  <span className="text-[10px] font-bold uppercase tracking-tighter">AI Reasoning Process</span>
                </div>
                {message.reasoning}
              </div>
            )}
        ...
        <div 
          className={`px-5 py-4 rounded-2xl shadow-xl text-[15px] leading-relaxed border ${
            isUser 
              ? 'bg-cyan-700 text-white rounded-tr-none border-cyan-600' 
